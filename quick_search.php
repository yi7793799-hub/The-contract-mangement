...
<?php if ($searched): ?>
<div class="mf-panel"><div class="mf-table-wrap"><table class="mf-table mf-table--striped table-mf mf-mb-0"><thead><tr><th>合同编号</th><th>合同名称</th><th>款项类型</th><th>客户名称</th><th>合同类型</th><th>状态</th><th>合同金额</th><th>截止日期</th></tr></thead><tbody><?php foreach($rows as $r): ?><tr><td><?= e((string)$r['contract_no']) ?></td><td><a href="<?= e(url('contract_view.php?id='.(int)$r['id'])) ?>"><?= e((string)$r['contract_name']) ?></a><?= mf_subcontract_tag((string)$r['contract_name'], (int)($r['is_subcontract'] ?? 0)) ?></td><td><?= mf_payment_type_badge((string)($r['payment_type'] ?? 'receipt')) ?></td><td><?= e((string)$r['customer_name']) ?></td><td><?= e((string)($r['type_name']?:'-')) ?></td><td><?= mf_contract_status_badge((string)$r['status']) ?></td><td>¥<?= number_format((float)$r['amount'],2) ?></td><td><?= e((string)($r['expiry_date']?:'-')) ?></td></tr><?php endforeach; ?><?php if(!$rows): ?><tr><td colspan="8" class="mf-text-center mf-text-muted mf-p-4">未搜索到合同数据</td></tr><?php endif; ?></tbody></table></div></div>
<?php endif; ?>
<?php
